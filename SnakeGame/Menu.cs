...
";
        ...
        {
            countdownSound.Play();
            var emptyBar = new string(' ', 100);
            var offset = new string(' ', (Console.WindowWidth - emptyBar.Length) / 2);
            var text = $@"
                                                        Loading...
...
{offset}{emptyBar.PastelBg(BROWN)} 0 %";
            ...
            for (int i = 1; i <= 100; i++)
            {
                ...
                var emptyPart = new string(' ', 100 - i).PastelBg(BROWN);
